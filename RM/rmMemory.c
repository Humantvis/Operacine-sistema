...
int initialize_RM_memory (RM_Memory* memory) {
    memory->userMemory = calloc(USER_MEMORY_SIZE * PAGE_SIZE, sizeof(uint8_t));
    if (!memory->userMemory) {
        perror("userMemory allocation failed");
        return 1;
    }

    memory->supervizorMemory = calloc(SUPERVIZOR_MEMORY_SIZE * PAGE_SIZE, sizeof(uint8_t));
    ...
    memory->sharedMemory = calloc(SHARED_MEMORY * PAGE_SIZE, sizeof(uint8_t));
